...
## read in data
# results <- lapply(list.files(paste0(copula, "/power_ar"), full.names = TRUE), readRDS)
results <- lapply(list.files(paste0(copula, "/power_ar"), pattern = "ar2", full.names = TRUE), readRDS)
...
mapply(function(n, di, li, cov){
  ...
  ggsave(paste0("plots/", copula, "/power_ar/power_ar_", li, "_", n, "_dim_", di,"_cov_", cov, ".pdf"), plotI, width = 12, height = 9)
...
mapply(function(n, di, li, cov){
  plotI <- ggplot(subset(settings, obs == n & dim == di & link == li & covariate == cov)) +
    ...
    geom_hline(yintercept = 0.05) + theme_bw() + 
    labs(x = "Parameter Value", y = "Empirical Power", col = "Tested Parameter") +
    scale_color_manual(labels = c("beta" = expression(beta), "covariate" = expression(gamma)),
                       values = c("red", "yellow")) + 
    theme(legend.position = "bottom",
          text = element_text(size = 30))
  ggsave(paste0("plots/", copula, "/power_ar/power_ar2_", li, "_", n, "_dim_", di,"_cov_", cov, ".pdf"), plotI, width = 12, height = 9)
}, n = set_log$obs, li = set_log$link, di = set_log$dim, cov = TRUE)
