...
# replicate=1-40
settings <- expand.grid(dim = c(10, 20, 30, 40, 50),
                        link = c("log", "identity"),
                        obs = c(5, 10, 20, 50, 100),
                        covariate = c(TRUE),
                        test = c("beta", "covariate"),
                        stringsAsFactors = FALSE)
settings <- subset(settings, !(!covariate & test == "covariate"))
settings <- settings[order(settings$dim, settings$link, settings$obs),]
...
if(settings$covariate[index]){
  ...
  if(settings$link[index] == "log"){
    params <- list(intercept = 0.6, past_obs = c(0.2, 0.25), covariates = 0.9)
  } else {
    params <- list(intercept = 5, past_obs = c(0.2, 0.25), covariates = 2)
  }
} else if(!settings$covariate[index]){
  model <- list(intercept = "homo", past_obs = 1)
  if(settings$link[index] == "log"){
    params <- list(intercept = 0.6,  past_obs = c(0.2, 0.25))
  } else {
    params <- list(intercept = 5, past_obs = c(0.2, 0.25))
  }
}
...
saveRDS(results, file = paste0("asymptotics_dim/asymptotics_dim_sim_setting_", index, ".rds"))
