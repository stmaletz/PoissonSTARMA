...
if(settings$covariate[index]){
  ...
  if(settings$link[index] == "log"){
    params <- list(intercept = 0.6, past_obs = c(0.2, 0.1), covariates = 0.9)
  } else {
    params <- list(intercept = 5, past_obs = c(0.2, 0.1), covariates = 2)
  }
...
saveRDS(results, file = paste0("asymptotics_ar/asymptotics_ar_sim_setting_", index, ".rds"))
